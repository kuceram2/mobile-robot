  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints)
  {
   ...
   // Check number of gpio components (2)
    if (info_.gpios.size() != 2)
  {
    RCLCPP_FATAL(
      rclcpp::get_logger("DiffDriveArduinoHardware"), "RRBotSystemWithGPIOHardware has '%ld' GPIO components, '%d' expected.",
      info_.gpios.size(), 1);
    return hardware_interface::CallbackReturn::ERROR;
  }
  // check numbler of gpio cmd interfaces (1) 
    if (info_.gpios[0].command_interfaces.size() != 1)
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "GPIO component %s has '%ld' command interfaces, '%d' expected.",
        info_.gpios[0].name.c_str(), info_.gpios[0].command_interfaces.size(), 1);
      return hardware_interface::CallbackReturn::ERROR;
    }
  // Check number of state interfaces (1)
    if (info_.gpios[0].state_interfaces.size() != 1)
  {
    RCLCPP_FATAL(
      rclcpp::get_logger("DiffDriveArduinoHardware"), "GPIO component %s has '%ld' state interfaces, '%d' expected.",
...
std::vector<hardware_interface::StateInterface> DiffDriveArduinoHardware::export_state_interfaces()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "State interfaces:");
    ...
    for (size_t i = 0; i < info_.gpios.size(); i++)
    {
      for (auto state_if : info_.gpios.at(i).state_interfaces)
      {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger("DiffDriveArduinoHardware"), "Added %s/%s", info_.gpios.at(i).name.c_str(), state_if.name.c_str());
          //&hw_gpio_in_[ct++]
      }
    }
...
std::vector<hardware_interface::CommandInterface> DiffDriveArduinoHardware::export_command_interfaces()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Command interfaces:");
    ...
    for (size_t i = 0; i < info_.gpios.size(); i++)
    {
      for (auto command_if : info_.gpios.at(i).command_interfaces)
      {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger("DiffDriveArduinoHardware"), "Added %s/%s", info_.gpios.at(i).name.c_str(), command_if.name.c_str());
      }
    }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  //comms_.set_digital_out(8, 0);
  comms_.set_digital_out(8, (int)hw_gpio_out_[0]);
  comms_.set_digital_out(7, (int)hw_gpio_out_[1]);
  return hardware_interface::return_type::OK;
}
